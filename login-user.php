...
if(isset($_POST['login'])){
    ...
    $cekdatabase = mysqli_query($conn, "SELECT * FROM userlogin where email = '$email' and password = '$password'");

    // Menghitung jumlah data
    $hitung = mysqli_num_rows($cekdatabase);
    if($hitung > 0){
        $_SESSION['log'] = 'True';
        echo '<script>alert("Login berhasil!"); window.location.href="index-user.php";</script>';
    } else {
        echo '<script>alert("Login gagal!"); window.location.href="login-user.php";</script>';
    }
}
...
<html lang="en">
    <head>
        ...
        <title>Login User - Yuk Ngestok Membantu Bisnis Anda Lebih Mudah</title>
                                    ...
                                    <div class="card-body">
                                        <form method="post">
                                            ...
                                            <div class="form-group d-flex align-items-center justify-content-between mt-4 mb-0">
                                                ...
                                                <a class="btn btn-warning" href="index-user.php">Login Admin</a>
                                            </div>
                                        </form>
                                    </div>
                                    <div class="card-footer text-center">
                                        <div class="small"><a href="register-user.php">Belum punya akun User? Buat disini!</a></div>
                                    </div>
                                </div>
                            </div>
                        </div>
